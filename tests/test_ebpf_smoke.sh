...
if [ "$(id -u)" -ne 0 ]; then
    # Check if sudo -n works without password
    if ! sudo -n true 2>/dev/null; then
        echo "Skipping eBPF test: root or passwordless sudo required."
        exit 125
    fi
fi

# Check if eBPF tracepoint is accessible
if ! sudo -n cat /sys/kernel/tracing/available_filter_functions >/dev/null 2>&1; then
    echo "Skipping eBPF test: tracing filesystem not accessible."
    exit 125
fi

# Run the trace with eBPF backend for a short time on /bin/ls
output=$(sudo -n "$RUNTIMEXRAY_BIN" trace --backend ebpf --timeout 1 /bin/ls 2>&1)
status=$?
if [ $status -ne 0 ]; then
